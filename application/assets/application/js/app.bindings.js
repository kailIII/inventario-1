//CUSTOM BINDING PARA CHECKBOX BUTTONS DE BOOTSTRAP
ko.bindingHandlers.checkboxButtons = {
	init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
		$(element).children('.btn').on('click',function(event){
			var v = $(this).val(),
				value = valueAccessor();
	...
	update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
		...
		$(element).children('.btn').each(function(i,e){
...
//SE REQUIERE EL PLUGIN DE DATATABLES PARA HACER FUNCIONAR ESTE DATABINDING http://datatables.net/
//SE REQUIERE EL PLUGIN PARA BOOTSTRAP DE DATATABLES PARA HACER FUNCIONAR ESTE DATABINDING http://datatables.net/plug-ins/pagination
...
//BINDING PARA LOS SELECT2
//SE REQUIERE EL PLUGIN SELECT2
...
ko.bindingHandlers.select2 = {
    init: function(element, valueAccessor, allBindingsAccessor) {
        ...
        $(element).select2(obj).on('change',obj.change);
...
//BINDING PARA LOS RADIOBUTTONS
ko.bindingHandlers.radioButtons = {
    init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        var value = valueAccessor();
        $(element).children('.btn').click(function(event){
    ...
    update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        $(element).children('.btn').each(function(i,e){
            $(e).removeClass('active');
            if($(e).val() == value()){ $(e).addClass('active'); }
        });
    }
};


//BINDING PARA DATEPICKER
//SE REQUIERE DATEPICKER PARA BOOTSTRAP
//aplicacion: application/assets/bootstrap_datepicker
//oficial: https://github.com/eternicode/bootstrap-datepicker
//
//Datepicker normal, formato del binding:
// data-bind="datepicker:{opciones del datepicker}"
...
//Datepicker inline, formato del binding:
// data-bind="datepicker_inline:{opciones:{ opciones del datepicker },observable:ObservableKnockout}"
ko.bindingHandlers.datepicker_inline = {
    init: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext){
        var value = valueAccessor();
        $(element).datepicker(value.opciones).on('changeDate', function(e){
            if(value.opciones.format){
                value.observable(e.format(value.opciones.format));
            }else{
                value.observable(e.date);
            }
        });
    },
    update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext){
        var value = valueAccessor();
        $(element).datepicker('update', value.observable());
    }
};