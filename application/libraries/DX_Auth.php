...
class DX_Auth
{
	...
	*/
	...
	function _set_session($data)
	{
		...
		$user = array(						
			'DX_user_id_parent'					=> $data->id_user,
			'DX_user_id'						=> $data->id,
			'DX_username'						=> $data->username,
			'DX_role_id'						=> $data->role_id,			
			'DX_domain'						=> $data->domain,			
	...
	*/
	...
	{
		if($this->ci->uri->segments){
			$this->ci->uri->segments = array_slice($this->ci->uri->segments,0,5);
			list($this->module, $controller, $this->method) = array_pad($this->ci->uri->segments,5,NULL);
		}else{
			$this->module = false;

		}
		
 		if($this->is_logged_in() and !$this->module)
    	$this->module = $this->get_domain();
		
		if($this->module and $this->module!="null" and !$this->is_superadmin()){

			$this->ci->db->select('uc.id');
			$this->ci->db->select('uc.id_user');
			$this->ci->db->select('uc.domain');
			$this->ci->db->select('uinfo.company');
			$this->ci->db->select('uinfo.description');
			$this->ci->db->select('uinfo.logo');
			$this->ci->db->from('app_users_client as uc');
			$this->ci->db->join('app_user_info as uinfo','uc.id=uinfo.id_user','left');
			$this->ci->db->where('uc.domain',$this->module);
			$this->ci->db->where('uc.role_id',2);
			$this->ci->db->where('uc.id_user',0);
			$consulta = $this->ci->db->get();
			//echo$this->ci->db->last_query();
			//pr($consulta->result_array());
			if ($consulta->num_rows() > 0){
				foreach ($consulta->result_array() as $row){
					
		    		$row['logo_'] = logo($row['logo'],$row["id"]);
				...
				//show_404("",true);
				return $data=array("id"=>0,"id_user"=>0,"domain"=>"","company"=>"","description"=>"");
			}
		}else{
	...
	function get_user_id_parent()
	{
		return $this->ci->session->userdata('DX_user_id_parent');
	}	
	// Get user id
	...
	function get_domain()
	{
		return $this->ci->session->userdata('DX_domain');
	}	
	// Get username string
	...
	// Checamos si es superadministrador
	function is_superadmin(){
		return strtolower($this->ci->session->userdata('DX_role_name')) == 'super';	
	}
	...
	function is_admin()
	{
		return strtolower($this->ci->session->userdata('DX_role_name')) == 'administrador';
	}
	...
	function register($username,$password,$domain, $email,$idRole,$id_subsidiary)
	{
		...
		// Default return value
		$result = FALSE;
		$id_user__ = $this->getUserDomain();
		
		if($idRole==2 and !$id_user__["id"])
		$id_user__=array("id"=>0,"domain"=>"");

		// New user array
		$new_userClient = array(			
			// 'id_user'				=> $id_user__["id"],
			'username'				=> $username,
		...
		if ($this->ci->config->item('DX_email_activation'))
		{
			...
			$new_userClient['activation_key'] = md5(rand().microtime());
			
			// Create temporary user in database which means the user still unactivated.
			$insert = $this->ci->user_temp->create_temp($new_userClient);
		}
		else
		{				
			// por el momento se guardan en la misma tabla, ambos usuarios
			if($idRole==2 and !$id_user__["id"]):
			$insert = $this->ci->users->createUserClient($new_userClient);
			else:
			$insert = $this->ci->users->create_user($new_user);
			endif;
			// Perfil del usuario
			$this->user_activated($this->ci->db->insert_id());
			//if($idRole==2) //logo del usuario, solo se crea si es administrador
			//$this->user_info($this->ci->db->insert_id());
		}
		
		if ($insert)
		{
			// Replace password with plain for email
			$new_userClient['password'] = $password;
			...
			if ($this->ci->config->item('DX_email_activation'))
			{
				...
				// Activation Link
				$new_userClient['activate_url'] = site_url($this->ci->config->item('DX_activate_uri')."{$new_userClient['username']}/{$new_userClient['activation_key']}");
				
				// Trigger event and get email content
				$this->sending_activation_email($new_userClient, $message);
			...
			else
			{
				...
				if ($this->ci->config->item('DX_email_account_details')) 
				{
					...
					// Trigger event and get email content
					$this->sending_account_email($new_userClient, $message);
	...
	function user_info($user_id)
	{
		...
		// Se creo el logo del usuario
		$this->ci->user_profile->create_info($user_id);
	}	
