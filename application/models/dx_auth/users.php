...
class Users extends CI_Model 
{
	function __construct()
	{
		...
		if($this->dx_auth->is_superadmin())
		$this->id_user["id_user"] = 0;
		if($this->dx_auth->is_admin())
		$this->id_user = $id_user__;
		//$this->id_user["id_user"] = $this->dx_auth->get_user_id();
	...
	function get_all($offset = 0, $row_count = 0)
	{
		...
		if ($offset >= 0 AND $row_count > 0)
		{
			...
			$this->db->where("$users_table.id_user",$this->id_user["id_user"]);
	...
	{
		$this->db->select('1', FALSE);
		$this->db->where('LOWER(domain)=', strtolower($domain));
		return $this->db->get($this->_table);
	}		
	...
	// User table function
	function createUserClient($data)
	{
		$data['created'] = date('Y-m-d H:i:s', time());
		$data['modified_pass'] = date('Y-m-d H:i:s', time());
	...
	function set_userInfo($data)
	{
		$this->db->where('id_user', isset($this->id_user["id_user"])?$id_user["id_user"]:"");
		return $this->db->update("app_user_info", $data);
	}	
	...
	function change_password($user_id, $new_pass)
	{
		...
		$this->db->set('modified_pass', date('Y-m-d H:i:s', time()));
		$this->db->where('id', $user_id);
		return $this->db->update($this->_table);
	}
}
